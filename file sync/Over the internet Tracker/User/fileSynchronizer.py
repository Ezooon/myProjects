...
class FileSynchronizer(threading.Thread):
	...
	def sync(self):
		...
		# Create list to store current file names. This makes it
		...
		for file in newFiles:
			try:
				self.fileToDelete = file # used for keeping track of file to delete incase of program close
				print("\n\nGetting file: ",file, "...")
				f = open(file,"wb")
				...
				os.remove(file)
			self.fileToDelete = None # Reset to none if sucessful

		t = threading.Timer(3, self.sync) #Step 5. start timer
		t.start()
sys.argv = [sys.argv[0], "192.168.2.254", "9000"]
